...
import { NativeElement, setTextContentNull, getInnerHtml } from "../index";
...
export function replaceLastChildAndUnmount(
  ...
  replaceVNode(
    parentDom,
    mount(nextInput, null, lifecycle, context),
    lastInput,
    lifecycle,
    isRecycling
  );
...
export function documentCreateElement(tag): NativeElement {
  // if (isSVG === true) {
...
export function replaceWithNewNode(
  ...
  const dom = mount(nextNode, null, lifecycle, context);
...
export function isSameInnerHTML(dom: NativeElement, innerHTML: string): boolean {
  const tempdom = document.createElement("i");

  tempdom.innerHTML = innerHTML;
  return tempdom.innerHTML === getInnerHtml(dom)
}
